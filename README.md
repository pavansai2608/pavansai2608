...
🎫 **[Complaint Classification & Resolution System](https://github.com/pavansai2608/complaint-classification-system)** — React + Node/Express + MongoDB with a Python FastAPI service that reads every incoming complaint: a TF-IDF/logistic-regression classifier (**78.8% accuracy, 0.79 macro-F1** on a held-out set) that returns its own confidence and defers low-confidence cases to a human, DistilBERT sentiment, an explainable rules engine for priority, and Gemini-drafted replies an agent reviews before sending. 227 tests across four suites, Docker, Kubernetes, Jenkins. [Live example, no account needed](https://3-105-252-57.sslip.io).

📊 **[Customer Segmentation & Retention](https://github.com/pavansai2608/customer-segmentation-retention)** — RFM + K-Means segmentation, BG/NBD + Gamma-Gamma lifetime value, and XGBoost churn prediction (ROC-AUC 0.788) over 542K transactions, combined into a ranked retain / nurture / let-go decision for 4,334 customers. FastAPI + React, MLflow, DVC, live on Render.

🛒 **[MSME Marketplace](https://github.com/pavansai2608/msme-marketplace)** — Built a hybrid recommendation service as its own FastAPI process (TF-IDF content similarity blended with co-purchase filtering), replacing a feature advertised as "XGBoost forecasting" that was a hardcoded constant. Inherited a four-person codebase, audited it end to end, fixed the access-control and data-exposure bugs it surfaced, took the project from zero tests to 133, and deployed three services on AWS k3s with automatic TLS.
